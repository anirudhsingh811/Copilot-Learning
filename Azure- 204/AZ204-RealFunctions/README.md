...
### ? "QueueNotFound" or "ContainerNotFound" errors (404)

**Symptom:** Continuous error logs like:
```
Error response [xxx] 404 The specified queue does not exist.
Error response [xxx] 404 The specified container does not exist.
```

**Cause:** Azurite is running, but the queues and blob containers haven't been created yet.

**Solution:** Run the setup script to create all required storage resources:
```powershell
.\Setup-LocalStorage.ps1
```
...
After creating resources, the errors will stop immediately!
...
- Stop other function apps: `Get-Process -Name func | Stop-Process`
...
- Ensure queue/blob containers exist (see "QueueNotFound" solution above)
- Check timer CRON expression syntax
...
- [ ] Start Azurite (`azurite --silent --location c:\azurite`)
- [ ] **Create storage resources** (`.\Setup-LocalStorage.ps1`) ?? **IMPORTANT!**
