...
namespace AzureAppServiceExamples
{
    ...
    public class AppServiceManager
    {
            ...
            string webAppName)
        {
            ...
            // In production, use Azure Portal, CLI, or proper SDK methods
            Console.WriteLine("App settings should be configured via:");
            Console.WriteLine("  - Azure Portal: Configuration > Application settings");
            Console.WriteLine("  - Azure CLI: az webapp config appsettings set");
            Console.WriteLine("  - Or use WebSiteResource.GetApplicationSettingsAsync()");
        ...
        /// Note: Scaling is best done via Azure CLI or Portal for the current SDK version
        /// </summary>
            ...
            int capacity = 2)
        {
            ...
            Console.WriteLine($"Current App Service Plan: {appServicePlan.Value.Data.Name}");
            Console.WriteLine($"Current SKU: {appServicePlan.Value.Data.Sku.Name}");
            Console.WriteLine();
            Console.WriteLine($"To scale, use Azure CLI:");
            Console.WriteLine($"  az appservice plan update --name {appServicePlanName} --resource-group {resourceGroupName} --sku {skuName}");
            
            // Note: UpdateAsync with patch data has API signature issues in current SDK version
            // Use Azure CLI or Portal for production scaling
        }
        ...
        /// Note: Slot swapping is best done via Azure CLI or Portal for the current SDK version
        /// </summary>
            ...
            string targetSlot = "production")
        {
            ...
            Console.WriteLine();
            Console.WriteLine($"To swap slots '{sourceSlot}' with '{targetSlot}', use Azure CLI:");
            Console.WriteLine($"  az webapp deployment slot swap --name {webAppName} --resource-group {resourceGroupName} --slot {sourceSlot} --target-slot {targetSlot}");
            
            // Note: Swap methods have API compatibility issues in current SDK version
            // Use Azure CLI or Portal for production slot swapping
        }
